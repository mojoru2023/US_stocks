...
if __name__ == '__main__':
    ...
    count_sql = "select count(*) from us_FinData_sina; "
    cur.execute(count_sql)
    long_count = cur.fetchone()['count(*)']
    # sql 语句
    big_list = []
    for num in range(1, long_count):
        sql = 'select name,d1,d2,d3,d4,d5  from us_FinData_sina where id = %s ' % num
        # #执行sql语句
        cur.execute(sql)
        # #获取所有记录列表
        data = cur.fetchone()
...
# name,d1,d2,d3,d4,d5
    ...
    l_path = os.getcwd()
    csv_dict_write('{0}/nas100_Fins.csv'.format(l_path), head, big_list)
    print("数据导出完成～")