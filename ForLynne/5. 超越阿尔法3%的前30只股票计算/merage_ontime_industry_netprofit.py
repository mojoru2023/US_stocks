...
def top2index_top30_and_last30_stock(tablename,dbname):
...
# 4 返回代码和带代码的收益率
    ...
    top30_item_dict = sort_data_top30[:30]
    last30_item_dict = sort_data_last30[:30]

    # top30 last30 code
    top30_code_list = sort_data_top30[:30].keys().tolist()
    last30_code_list = sort_data_last30[:30].keys().tolist()
...
def output_dt_use_code(code_list,excelfilename,dbname,tablename):
    fordb_select_code = ",".join(code_list)
    engine_Mons = create_engine('mysql+pymysql://root:123456@localhost:3306/{0}'.format(dbname))
    sql_f = 'select {0} from {1}  ; '.format(fordb_select_code,tablename)
    df_f = pd.read_sql_query(sql_f.format(",".join(get_code)), engine_Mons)
    df_f.to_excel("{0}-{1}.xlsx".format(excelfilename,tablename))



def merge_3table(tablename,itemlist):
...
def load_ontimedt_and_detailsDt(dbname,tablename):
    top30_code_list, last30_code_list, top30_item_dict, last30_item_dict = top2index_top30_and_last30_stock(tablename, dbname)
    output_dt_use_code(top30_code_list, "top30", dbname, tablename)
    output_dt_use_code(last30_code_list, "last30",dbname,tablename )
    merge_3table(top30_item_dict,tablename)
    merge_3table(last30_item_dict,tablename)
...
if __name__== "__main__":
    # 这里导出top30,last30数据
    dbname = "ForLynne"
    tablename = "following_sp500_plus_nas100_20220412"
    load_ontimedt_and_detailsDt(dbname,tablename)
