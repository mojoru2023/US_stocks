...
def insertDB(content):
    ...
    try:
        ...
        f_10 = "%s," *10
        cursor.executemany('insert into us_top10_0723 ({0},top10_ave) values ({1},%s)'.format(US_top10_DB,f_10[:-1]), content)
        connection.commit()
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
...
if __name__ == '__main__':
    ...
    US_top10_WEB ='ORCL,ALB,MAA,AAPL,ESS,AVB,CHKP,EQR,UDR,AMZN'

    US_top10_DB = 'ORCL,ALB,MAA,AAPL,ESS,AVB,CHKP,EQR,UDR,AMZN'
 ...
 # create table us_top10_0723(id int not null primary key auto_increment,  ORCL FLOAT,ALB FLOAT,MAA FLOAT,AAPL FLOAT,ESS FLOAT,AVB FLOAT,CHKP FLOAT,EQR FLOAT,UDR FLOAT,AMZN FLOAT,top10_ave FLOAT, LastTime timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP) engine=InnoDB  charset=utf8;
