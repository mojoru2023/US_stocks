...
def parse_html(html):  # 正则专门有反爬虫的布局设置，不适合爬取表格化数据！
    big_list = []
    # patt = re.compile('<span class="Trsdu(0.3s) Fw(b) Fz(36px) Mb(-4px) D(ib)" data-reactid="32">(.*?)</span>',re.S)
    # items = re.findall(patt,html)
    selector = etree.HTML(html)
    Price = selector.xpath('//*[@id="quote-header-info"]/div/div/div/span/text()')
