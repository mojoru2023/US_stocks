...
def insertDB(content):
    ...
    try:
        ...
        f_104 = "%s," *104
        cursor.executemany('insert into nas100_2022_02_23 ({0}) values ({1})'.format(nasdap100,f_104[:-1]), content)
        connection.commit()
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
